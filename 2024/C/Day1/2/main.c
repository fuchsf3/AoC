...
short numberContainedInSotedList(int *list, int size, int number) {
    int left = 0;
    int right = size - 1;
    int mid = 0;
    int count = 0;
    ...
    while (left <= right) {
        ...
        if (list[mid] == number) {
            ...
            left = mid - 1;
            right = mid + 1;
            while (list[left] == number) {
                count++;
                left--;
            }
            while (list[right] == number) {
                count++;
                right++;
            }
            return count;
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < count; i++) {
        total += list1[i] * numberContainedInSotedList(list2, count, list1[i]);
    }
